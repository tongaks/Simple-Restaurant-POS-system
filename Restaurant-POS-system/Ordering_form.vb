...
Public Class Order
    ...
    Private Sub Order_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        'MenuContainerPnl.Controls(0).BackColor = Color.SteelBlue
        'MenuContainerPnl.Controls(0).ForeColor = Color.White
    ...
    Private Sub SearchItem(itemName As String)
        ...
        Try
            ...
            Dim Query = "SELECT * FROM (SELECT * FROM `restaurant`.foods UNION ALL SELECT * FROM `restaurant`.drinks UNION ALL SELECT * FROM `restaurant`.snacks_sides) AS CombinedItems WHERE ItemName LIKE CONCAT('%', @itemName, '%')"
            Dim Command As New MySqlCommand(Query, Connection)
        ...
        Catch ex As Exception
            MsgBox("ERROR search: " & ex.Message.ToString, MsgBoxStyle.Critical, "error")
        End Try
    End Sub
    ...
    Private Sub ApplyDiscount_Click(sender As Object, e As EventArgs) Handles DiscountBtn.Click
        ...
        If applyVoucherForm.ShowDialog() = DialogResult.OK Then
            ...
            For Each cntrl As Control In applyVoucherForm.DiscountPnl.Controls
                If TypeOf cntrl Is Guna2TextBox Then
                    Dim txtBox As Guna2TextBox = CType(cntrl, Guna2TextBox)
    ...
    Private Sub SearchTxtBox_TextChanged(sender As Object, e As EventArgs) Handles SearchTxtBox.TextChanged
        If SearchTxtBox.Text = "" Then
            ...
            If MenuCategories.Count > 0 Then
                HandleCategorylick(MenuCategories(0), Nothing)
    ...
    Private Sub HandleIfItemExistsInOrder(ByVal itemName As String, ByVal itemPrice As String, ByVal tagImgPath As String, ByVal itemAmount As Integer)
        ...
        For Each row As DataGridViewRow In DataGridView1.Rows
            If row.Cells(1).Value IsNot Nothing AndAlso row.Cells(1).Value.ToString() = itemName Then
                ...
                row.Cells(3).Value = CInt(row.Cells(3).Value) + Double.Parse(itemPrice) ' set item total
                Exit For
            End If
        Next
    ...
    Private Sub Compute()
        ...
        For Each row As DataGridViewRow In DataGridView1.Rows
            ...
            Dim itemPrice As Double = Double.Parse(row.Cells(2).Value) ' item price
    ...
    Private Sub HandleDiscount(ByVal itemPrice As String)
        CurrentSubTotal += Double.Parse(itemPrice)
        SubtotalLbl.Text = "₱" & If(CurrentSubTotal - Math.Floor(CurrentSubTotal) >= 0.5, Math.Ceiling(CurrentSubTotal), Math.Floor(CurrentSubTotal))

        Dim appliedDiscount = (DiscountValue * CurrentSubTotal)
        CurrentTotal = If((Not DiscountValue = 0), Double.Abs(appliedDiscount - CurrentSubTotal), CurrentSubTotal)
        TotalLbl.Text = "₱" & If(CurrentTotal - Math.Floor(CurrentTotal) >= 0.5, Math.Ceiling(CurrentTotal), Math.Floor(CurrentTotal))
    End Sub
